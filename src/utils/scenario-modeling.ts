...
interface DriverOutcomeRelationships {
  [key: string]: string[];
}
...
// Define driver-outcome relationships
export const driverOutcomeRelationships: DriverOutcomeRelationships = {
  // Cost Drivers
  "cost.labor": ["ebitda", "laborCost"],
  "cost.energy": ["ebitda", "energyCost"],
  "cost.maintenance": ["ebitda", "maintenanceCost"],
...
export const impactFactors: ImpactFactors = {
...
export function calculateOutcomes(driverValues: Record<string, number>) {
  const outcomes: Record<string, MetricDefinition> = { ...outcomeMetrics }

  // Initialize outcome values with baseline
  Object.keys(outcomes).forEach((key) => {
    const metric = outcomes[key]
    if (metric) {
  ...
  Object.entries(driverValues).forEach(([driverId, value]) => {
    ...
    affectedOutcomes.forEach((outcomeId: string) => {
      ...
      if (outcome) {
        ...
        const impact = (value / 100) * impactFactor * outcome.baseline
        outcome.value = (outcome.value || outcome.baseline) + impact
        outcome.percentChange = ((outcome.value || outcome.baseline) / outcome.baseline - 1) * 100
      }
    })
  })
